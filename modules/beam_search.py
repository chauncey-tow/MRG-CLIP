...
class BeamSearch(object):
    ...
    def sample_beam(self, att_feats, att_masks=None):
        ...
        seq = att_feats.new_full((batch_size * self.sample_n, self.args["max_seq_length"]-2), self.args["pad_idx"], dtype=torch.long)
        seqLogprobs = att_feats.new_zeros(batch_size * self.sample_n, self.args["max_seq_length"]-2, self.vocab_size + 1)
    ...
    def beam_search(self, init_state, init_logprobs, *args):
        ...
        for t in range(self.args["max_seq_length"]-2 + group_size - 1):
            for divm in range(group_size):
                if t >= divm and t <= self.args["max_seq_length"]-2 + divm - 1:
                    # add diversity
                    ...
                    for b in range(batch_size):
                        ...
                        if t == self.args["max_seq_length"]-2 + divm - 1:
                            is_end.fill_(1)
